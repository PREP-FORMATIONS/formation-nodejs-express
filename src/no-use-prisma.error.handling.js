  ...
  if (err instanceof Prisma.PrismaClientValidationError) {
    ...
    switch (err.code) {
      ...
      case 'P2025':
        ...
        res.status(404).json({
          error: 'Not found',
          details: 'An operation failed because it depends on one or more records that were required but not found',
        });
        break;
      case 'P2026':
        ...
        res.status(400).json({
          error: 'Bad request',
          details: 'The current database provider doesn\'t support a feature that the query used',
        });
        break;
      case 'P2027':
        ...
        res.status(500).json({
          error: 'Internal server error',
          details: 'Multiple errors occurred on the database during query execution',
        });
        break;
      default:
        // Other Prisma errors
